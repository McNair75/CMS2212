...
<p>C\'est une balise administration qui fournit un contrôle pour permettre la sélection d\'une page de contenu, ou un autre élément. Ceci est approprié pour permettre à un administrateur du site de sélectionner une page qui sera stockée dans une préférence..</p>
...
<p>C\'est une balise pour afficher les éléments balises HTML < option > et < optgroup >. Chaque option peut avoir des éléments enfant, sa propre étiquette de titre, et si besoin son propre attribut de classe.</p>
<h3>Comment l\'utiliser ?</h3>
...
<p>C\'est une balise d\'administration uniquement, permettant aux modules d\'afficher facilement les icônes du thème d\'administration actuel. Ces icônes sont utiles dans la construction des liens ou pour afficher des informations d\'état.</p>
<h3>Quels paramètres ?</h3>
<ul>
...
<p>Ceci est une balise intelligente utile pour générer une URL sous l\'action d\'un module. Cette balise est utile pour les développeurs de modules qui génèrent des liens (que ce soit pour Ajax ou dans l\'interface administration) pour réaliser des fonctionnalités différentes ou afficher des données différentes.</p>
<h3>Quels paramètres ?</h3>
<ul>
...
<p><strong>Note :</strong> Tous les autres paramètres qui ne sont pas acceptés par cette balise sont automatiquement transmis à l\'action du module par l\'URL générée.</p>
<h3>Exemple :</h3>
...
<p>Cette balise administrateur donne une information sur l\'utilisateur "Admin" à partir de son Id.</p>
<h3>Quels paramètres ?</h3>
<ul>
...
<p>C\'est une balise d\'administration uniquement, utilisée pour générer un lien qui lorsqu\'il est cliqué va afficher une aide contextuelle pour un élément particulier.</p>
<p>Cette balise est généralement utilisée à partir des gabarits module d\'administration pour afficher un popup d\'aide utilisateur pour un champ de saisie, une colonne ou autres renseignements importants.</p>
<h3>Quels paramètres ?</h3>
<ul>
  ...
  <li>realm - <em>(option string)</em> - La première partie d\'une clé unique pour identifier la chaîne d\'aide. Si ce paramètre n\'est pas précisé et si cette balise est appelée à partir d\'une action de module alors le nom du module en cours est utilisé. Si aucun nom de module ne peut être trouvé alors "help" utilise la langue utilisateur.</li>
...
$lang['help_function_cms_pageoptions'] = '<h3>Que fait cette balise ?</h3>
 <p>C\'est un simple balise qui génère une séquence de balises "Option" pour une liste déroulante de pagination qui représente les numéros de page.</p>
 <p>En fonction du nombre de pages et de la page actuelle cette balise va générer une liste de numéros de pages qui permettent une navigation rapide vers un sous-ensemble des pages.</p>
<h3>Quels paramètres ?</h3>
  <ul>
...
<p>C\'est une balise simple, utilisée dans la génération de formulaire pour créer un ensemble d\'options < select > représentant un choix Oui/Non.</p>
<p>Cette balise va traduire le choix Oui/Non en une valeur préalablement choisie.</p>
<h3>Quels paramètres ?</h3>
<ul>
...
<p>Cette balise détecte et donne la langue que le navigateur accepte, et la compare à une liste des langues autorisées pour déterminer la langue de la session.</p>
<h3>Comment l\'utiliser ?</h3>
...
<ul>
  ...
  <li><strong>key (requis)</strong> La clé de l\'attribut retournée.
    <p> La clé peut être soit un nom de bloc, ou un ensemble de propriétés standard associées à une page de contenu. Quelques-unes des propriétés standard acceptées sont :</p>
    <ul>
      ...
      <li>pageattr - (string) La valeur de la donnée attribut Smarty spécifique à la page.</li>
     </ul>
<p><strong>Note :</strong> La liste ci-dessus n\'est pas exhaustive. Vous pouvez également récupérer les contenus non traités des blocs de contenu supplémentaires ou des propriétés ajoutées par des modules tiers. La sortie est toujours une chaîne de caractères.</p>
  <li><em>(option)</em> assign (string) - Affecte (Assigne) le résultat de la balise à la variable Smarty ainsi nommée.</li>
</ul>
<h3>Retours :</h3>
...
<p><strong>Note :</strong> - La sortie de cette balise ne passe pas par Smarty et n\'est pas nettoyée pour l\'affichage. Pour l\'affichage des données, vous devez convertir les chaînes de données en entités, et/ou les passer par Smarty.</p>';
...
<h3>Plus...</h3>
<p>Si l\'argument tag est activé et que la valeur de la propriété n\'est pas vide, cela déclenchera une balise HTML img complète à afficher. Tous les arguments de cette balise non listés ci-dessus seront automatiquement inclus dans la balise img résultante. Exemple : <code>{page_image tag=true class="pageimage" id="someid" title="testing"}</code>.</p>
<p>Si cette balise produit une balise img complète et que l\'argument alt n\'a pas été fourni, la valeur de la propriété sera utilisée pour générer l\'attribut alt de la balise img.</p>';
...
<p>En plus des arguments énumérés ci-dessus, cette balise acceptera tout arguments supplémentaires et les transmettra directement à la balise img générée, le cas échéant. Exemple : <code>{content_image block=\'img1\' id="id_img1" class="page-image" title=\'an image block\' data-foo=bar}</code>';
...
$lang['help_function_tab_end'] = '<h3>Que fait cette balise ?</h3>
  <p>Cette balise génère le code HTML pour indiquer la fin d\'une zone de contenu avec des onglets.</p>
<h3>Comment l\'utiliser ?</h3>
...
$lang['help_function_tab_header'] = '<h3>Que fait cette balise ?</h3>
  <p>Cette balise génère le code HTML pour l\'entête d\'un onglet dans une zone de contenu à onglets.</p>
<h3>Comment l\'utiliser ?</h3>
<p>Le code suivant crée une zone de contenu à onglets avec deux onglets.</p>
...
$lang['help_function_tab_start'] = '<h3>Que fait cette balise ?</h3>
  <p>Cette balise fournit le code HTML pour marquer le début du contenu d\'un onglet spécifique dans une zone de contenu à onglets.</p>
<h3>Comment l\'utiliser ?</h3>
<p>Le code suivant crée une zone de contenu à onglets avec deux onglets.</p>
<pre><code>{tab_header name=\'tab1\' label=\'Tab One\'}
{tab_header name=\'tab2\' label=\'Tab Two\'}
...
$lang['help_function_cms_textarea'] = '<h3>Que fait cette balise ?</h3>
  <p>Cette balise Smarty est utilisée lors de la construction des formulaires d\'administration pour générer un champ textarea. Il dispose de plusieurs paramètres qui permettent de vérifier si un plugin WYSIWYG <em>(si disponible)</em> ou un éditeur de syntaxe colorée sont utilisés , de contrôler le comportement de ces modules et la taille et l\'apparence du textarea.</p>
  <h3>Comment l\'utiliser ?</h3>
    <p>La façon la plus simple d\'utiliser cette balise est de spécifier <code>{cms_textarea name = "quelque_chose"}</code>. Cela va créer une zone de texte simple portant le nom spécifié, sans WYSIWYG ni syntaxe colorée.</p>
...
<p>Cette balise de fonction peut être utilisée pour fournir des instructions complémentaires (hint) pour contrôler le comportement du module si différents paramètres ne peuvent pas être indiqués sur l\'URL. Ex : Dans une situation où un site est configuré pour utiliser des "pretty" URLs pour le référencement, il est souvent impossible de fournir les paramètres supplémentaires des modules, comme un detailtemplate ou un ordre de tri sur une URL. Cette balise peut être utilisée dans les gabarits de page, blocs de contenu ou d\'une manière spécifique à la page pour donner des informations complémentaires quant à la façon dont les modules devraient se comporter.</p>
...
<p><strong>Note :</strong> Il est possible de spécifier plusieurs paramètres à un seul module dans un appel de cette balise.</p>
...
<p>Cette balise crée un champ de saisie contrôlé par le module <em>(courant)</em> "File Picker" pour permettre la sélection d\'un fichier. C\'est une balise Admin uniquement utile pour les gabarits des modules et aussi pour d\'autres formulaires d\'administration.</p>
...
<p>Note : cette balise détectera (par des mécanismes internes) le module "File Picker" qui peut être différent du module "Gestionnaire des fichiers" de CMSMS. Le module File Picker peut donc ignorer certains des paramètres du module "Gestionnaire des fichiers".</p>
<h3>Quels paramètres ?</h3>
<ul>
...
$lang['tag_about'] = 'Affiche des informations sur l\'historique et l\'auteur de cette balise(si disponible).';
...
$lang['tag_cachable'] = 'Indique si la sortie de cette balise peut être mise en cache (lorsque le cache Smarty est activé). Les balises Admin et les balises modifier ne peuvent pas être mis en cache.';
...
$lang['title_admin'] = 'Cette balise est disponible uniquement à partir de la console d\'administration de CMS Made Simple.';
$lang['title_notadmin'] = 'Cette balise est utilisable à la fois dans la console d\'administration et sur le site Web (frontend).';
$lang['title_cachable'] = 'Cette balise est mis en cache';
$lang['title_notcachable'] = 'Cette balise n\'est pas mis en cache';
