...
abstract class CmsAdminThemeBase {
     ...
     * @ignore
     */
    private $_valid_sections = array('main', 'content', 'layout', 'files', 'lise', 'usersgroups', 'extensions', 'siteadmin', 'ecommerce', 'myprefs');
    ...
    private function _FixSpaces($str) {
        ...
        if (!empty($tmp))
            ...
            return preg_replace('/\s+/', "&nbsp;", $str); // bad UTF8
    }
    ...
    private function _SetModuleAdminInterfaces() {
        ...
        if (!is_array($usermoduleinfo)) {
            ...
            foreach ($usermoduleinfo as $key => $obj) {
                ...
                $section = $obj->section;
                if (!in_array($section, $this->_valid_sections))
                    $section = 'extensions';
    ...
    private function _SetAggregatePermissions($force = FALSE) {
        ...
        // lise
        $this->_perms['lisePerms'] = (isset($this->_sectionCount['lise']) && $this->_sectionCount['lise'] > 0);
    ...
    private function _populate_admin_navigation($subtitle = '') {
        if ($this->_menuItems && !empty($this->_menuItems))
            return;
        ...
        // base lise menu ---------------------------------------------------------
        $items['lise'] = array('url' => 'index.php?section=lise', 'parent' => -1, 'priority' => 3,
            'title' => $this->_FixSpaces(lang('lise')), 'description' => lang('lisedescription'),
            'show_in_menu' => $this->HasPerm('lisePerms'));
        // base layout menu ---------------------------------------------------------
        ...
        // todo: cleanup
        foreach ($this->_menuItems as $sectionKey => $sectionArray) {
            if (!isset($this->_modulesBySection[$sectionKey]))
                continue;
        ...
        foreach ($this->_menuItems as $sectionKey => $sectionArray) {
            if (isset($sectionArray['system']) && $sectionArray['system'])
                continue;
            if ($sectionArray['parent'] == -1 && in_array($sectionKey, $this->_valid_sections))
                continue;
            if (isset($sectionArray['parent']) && in_array($sectionArray['parent'], $this->_valid_sections))
                continue;
        ...
        if ($selected_key || $pending_selected_key) {
            ...
            if (!$selected_key && $pending_selected_key)
                $selected_key = $pending_selected_key;
    ...
    protected function get_action_module() {
        if ($this->_action_module)
            return $this->_action_module;
    ...
    protected function get_module_help_url($module_name = null) {
        if (!$module_name)
            $module_name = $this->get_action_module();
        if (!$module_name)
            return;
        ...
        if (is_array($files) && count($files)) {
            ...
            foreach ($files as $file) {
                if (!is_dir($file))
                    continue;
                $name = basename($file);
                if (!is_readable(cms_join_path($file, "{$name}Theme.php")))
                    continue;
                $res[$name] = $name;
            }
            return $res;
    ...
    static public function GetThemeObject($name = '') {
        ...
        if (!$name)
            $name = cms_userprefs::get_for_user(get_userid(FALSE), 'admintheme', self::GetDefaultTheme());
