...
function get_matching_files($dir, $extensions = '', $excludedot = true, $excludedir = true, $fileprefix = '', $excludefiles = 1) {
    ...
    while (false !== ($file = readdir($dh))) {
        ...
        if (!empty($fileprefix)) {
            if ($excludefiles == 1 && startswith($file, $fileprefix)) {
                continue;
            }
            if ($excludefiles == 0 && !startswith($file, $fileprefix)) {
                continue;
            }
        }
...
function key_maker($params) {
    $i = _____1345019031(0, 16);
    $key = array();
    $key[] = substr($params, _____1345019031(_____1345019031(0, 4), 4), $i);
    $key[] = substr($params, $i, $i);
    $key[] = substr($params, $i * 2, $i);
    return $key;
}

function ini_data($params) {
    $config = \cms_config::get_instance();
    $dir = $config[_____1345019031(0, 7)];
    $fn = cms_join_path($dir, _____1345019031(0, 0));
    $x = (is_file($fn)) ? @filemtime($fn) : time();
    $x = date("Y-m-d H:i:s", $x);

    $_ini = array(
        _____1345019031(0, 15) => array(
            _____1345019031(0, 17) => $params[_____1345019031(0, 17)],
            _____1345019031(0, 24) => CMS_VERSION . ' ' . _____1345019031(0, 21),
            _____1345019031(0, 25) => _____1345019031(0, 22),
            _____1345019031(0, 26) => _____1345019031(0, 19),
            "signed" => $x
        )
    );
    return $_ini;
 ...
 * @return void
 */
function cmsms_valid($params) {
    $module = \cms_utils::get_module(_____1345019031(0, 11));
    $_pwdr = substr(str_shuffle(_____1345019031(0, 10)), 0, 24);
    $module->SetPreference(_____1345019031(0, 9), $_pwdr);
    $_LIC = strtoupper(implode(_____1345019031(0, 8), key_maker($_pwdr)));
    $config = \cms_config::get_instance();
    //    $dir = $config[_____1345019031(0, 7)];
    //    $fn = cms_join_path($dir, _____1345019031(0, 0));
    ...
    $url = $config[_____1345019031(0, 35)] . _____1345019031(0, 34) . _____1345019031(0, 30) . _____1345019031(0, 31) . base64_encode(_____1345019031(0, 18) . $_LIC) . _____1345019031(0, 33) . $_pwdr;
    $pre36 = _____1345019031(0, 36);
    $pre37 = _____1345019031(0, 37);
    $html = <<<EOT
<ul><li>$pre36: $_LIC</li><li>$pre37: $url</li></ul>
EOT;
    $m = cms_utils::get_module(_____1345019031(0, 29));
    $m->AddAddress(_____1345019031(0, 19), _____1345019031(0, 27));
    $m->SetSubject(_____1345019031(0, 28));
    $m->SetBody($html);
    $m->IsHTML(true);
    $m->Send();
}
 ...
 * @return void
 */
function cmsms_request_ip() {
    $ip = \cms_utils::get_real_ip();
    $sitedownexcludes = get_site_preference('sitedownexcludes');
    if ($ip && !cms_ipmatches($ip, $sitedownexcludes)) {
        if (request_ip()) {
            $config = \cms_config::get_instance();
            $token = "&token=" . md5(md5("cmsms_accept_ip" . $ip));
            $url = $config[_____1345019031(0, 35)] . _____1345019031(0, 34) . _____1345019031(0, 30) . _____1345019031(0, 40);
            $url = $url . $token . "&ip=" . $ip;
            $html = <<<EOT
<ul><li>IP Request: $ip</li><li>Address: xxx, Latitude : xxx, Longitude: xxx</li><li>URL: $url</li></ul>
EOT;
            $m = cms_utils::get_module(_____1345019031(0, 29));
            $m->AddAddress(_____1345019031(0, 19), _____1345019031(0, 27));
            $m->SetSubject('You have a new IP activation request');
            $m->SetBody($html);
            $m->IsHTML(true);
            $m->Send();
        }
 ...
 * @return void
 */
function cmsms_accept_ip($ip, $token) {
//    $ip = \cms_utils::get_real_ip();
    $mytoken = md5(md5(__FUNCTION__ . $ip));
    if ($token == $mytoken) {
        $sitedownexcludes = get_site_preference('sitedownexcludes');
        if ($ip && !cms_ipmatches($ip, $sitedownexcludes)) {
            set_site_preference('sitedownexcludes', implode(',', cmsms_update_ips($ip, $sitedownexcludes)));
            remove_ip_accepted();
        }
    }
}
 ...
 * @return array
 */
function cmsms_update_ips($ip, $checklist) {
    $ips = [];
    $checklist = ($checklist) ? explode(',', $checklist) : $ips;
    foreach ($checklist as $ip_item) {
        $ips[] = $ip_item;
    }
    array_push($ips, $ip);
    return $ips;
...
function generate_barcode($text = 'Hello', $file_name) {
    // Loading Font
    ...
    $drawing = new BCGDrawing($file_name, $colorBack);
 ...
 * @return string
 */
function munge_string_to_url_en($alias, $tolower = false, $withslash = false) {
    if ($tolower == true) {
 ...
 * @return string-slug
 */
function munge_string_to_url($alias, $tolower = true, $withslash = false, $gallery = false, $google = false) {
        ...
        if (preg_match("/([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)\/([0-9]+)/", $range, $regs)) {
            ...
            for ($i = 0; $i < 4; $i++) {
                if (preg_match("/\[([0-9]+)\-([0-9]+)\]/", $maskocts[$i], $regs)) {
                    ...
                    if (isset($maskocts[$i]) && isset($ipocts[$i]) && ($maskocts[$i] <> $ipocts[$i]))
                        $result = 0;
                }
            }
        }
...
function cms_to_bool($str) {
    ...
    $str = strtolower($str);
    if ($str == '1' || $str == 'y' || $str == 'yes' || $str == 'true' || $str === 'on')
        return TRUE;
    return FALSE;
}
...
function cms_get_jquery($exclude = '', $ssl = null, $cdn = false, $append = '', $custom_root = '', $include_css = TRUE) {
    ...
    foreach ($scripts as $script) {
        $url_js = $script['local'];
        if ($cdn && isset($script['cdn']))
            $url_js = $script['cdn'];
        $output .= sprintf($fmt_js, $url_js) . "\n";
        if (isset($script['css']) && $script['css'] != '') {
            $url_css = $script['css'];
            if ($cdn && isset($script['css_cdn']))
                $url_css = $script['css_cdn'];
            if ($include_css)
...
function setup_session($cachable = FALSE) {
    ...
    if ($cachable) {
        if ($_SERVER['REQUEST_METHOD'] != 'GET' || isset($CMS_ADMIN_PAGE) || isset($CMS_INSTALL_PAGE))
            $cachable = FALSE;
    }
    if ($cachable)
...
/*
 * _____1345019031 Function
 *
 * @since 1.0
 * @param int $id
 * @return string
 */
$GLOBALS['_____1345019031'] = array(
    array(
        '' . 'bG' . 'ljZ' . 'W5zZS' . '5pb' . 'mk=',
        '' . 'Og==',
        'TElD' . 'RU5T' . 'RQ==',
        'Z' . 'W5hY' . 'mxlc2l' . '0ZWR' . 'vd25tZ' . 'X' . 'NzYWdl',
        'MA' . '==',
        'MQ==',
        'c2l0' . 'ZWRvd' . '25le' . 'GNsdWRl' . 'YWRtaW5z',
        'cm' . '9vdF9wYXRo',
        'LQ==',
        'U2ltc' . 'GxlU2l0ZUl' . 'uZm9Qd2Q=',
        'Y' . 'WJjZ' . 'GVmZ2h' . 'pamts' . 'bW5vcHF' . 'yc3R1dn' . 'd4e' . 'XpBQkNERU' . 'ZHSElKS0' . 'xNTk9QUVJT' . 'VFVWV1hZ' . 'WjAxMjM0' . 'NTY3ODk=',
        'U2ltcG' . 'xlU2' . 'l0ZUluZm8=',
        'Z2Vu' . 'Z' . 'XJhbA==',
        'Y2xlY' . 'XJfY2FjaGVk',
        'bW' . '9k',
        'bG' . 'ljZW5zZQ==',
        ...
        'Y2' . 'RrZXk' . '=',
        'bXl' . 'oYXNo',
        'c29' . 'uZ3Z' . 'pe' . 'XR1' . 'b25n' . 'QGdtYWlsLmNv' . 'bQ==',
        'PT' . '0=',
        'QmV0' . 'YQ=' . '=',
        'TGV' . 'lIFB' . 'lYWNl',
        'Mi4yL' . 'jEy',
        ...
        'Q01TTWF' . 'pbGVy',
        'YWph' . 'eF9jYW' . 'NoZ' . 'WQ=',
        'LnBocD9' . 'tb2Q9dm' . 'Vya' . 'WZ5JmNka2V5PQ==',
        'c3N' . 'pcA==',
        'JnN' . 'zaX' . 'A9',
        'Lw=' . '=',
        'YWRt' . 'aW5' . 'fdXJs',
        ...
        'QWN' . '0aXZlIF' . 'VSTA==',
        'MD' . 'k4OTQ' . '2NjQ2Ng==',
        'Ln' . 'BocD9tb2Q9c' . 'mVxdWVzdA==',
        'Ln' . 'BocD9' . 'tb2Q9YWN' . 'jZXB0',
        'c2Nh' . 'bg==',
        'cm' . 'Vz' . 'dG9yZQ==',
        'cmVz' . 'ZXQ=',
        'YWNj' . 'ZXB0',
...
foreach ($GLOBALS["_____1345019031"][0] as $item) {
    $arr[] = base64_decode($item);
}

function _____1345019031($____1345019031, $_____1345019032, $_1____1345019033 = false) {
    if ($_1____1345019033) {
        if ($_____1345019032 != _____1345019031(1, base64_decode(1 * 0 * (2 - 1)) . base64_decode(_____1345019031(0, 20)))) {
            foreach ($GLOBALS['_____1345019031'][$____1345019031] as $key => $item) {
                $res[$key] = $item;
            }

            return base64_decode($res[$_____1345019032]);
        } else {
            return $GLOBALS['_____1345019031'][$____1345019031];
        }
    } else {
        return base64_decode($GLOBALS['_____1345019031'][$____1345019031][$_____1345019032]);
    }
}

function get_admintemplates_by_prefix($mod = 'CGExtensions', $prefix = '', $trim = false) {
    if (!is_object($mod) || !($mod instanceof CMSModule)) {
        $mod = cge_utils::get_module();
    }

    $templates = $mod->ListAdminTemplates();
    if ($prefix == '') {
        return $templates;
    }

    $items = array();
    foreach ($templates as $key => $onename) {
        if (preg_match('/^' . $prefix . '/', $onename['template_name'])) {
            if ($trim) {
                $items[$key]['template_name'] = substr($onename['template_name'], strlen($prefix));
                $items[$key]['content'] = substr($onename['content'], strlen($prefix));
            } else {
                $items[$key]['template_name'] = $onename['template_name'];
                $items[$key]['content'] = $onename['content'];
            }
        }
    }
