...
$item_query->AppendTo(LISEQuery::VARTYPE_WHERE, 'A.domain_id = ?');
$item_query->AppendTo(LISEQuery::VARTYPE_QPARAMS, $domain_id);
...
while ($result && $row = $result->FetchRow()) {
    ...
    if ($this->CheckPermission($this->_GetModuleAlias() . '_approve_item')) {
        ...
        if ($obj->active == -1) {
            $obj->approve = $admintheme->DisplayImage('icons/system/warning.gif', $this->ModLang('warning'), '', '', 'systemicon');
        } else {
            $obj->approve = $admintheme->DisplayImage('icons/system/accept.gif', $this->ModLang('published'), '', '', 'systemicon');
        }
    }
...
if ($pagenumber == 1) {
    ...
    $smarty->assign('prevpage', $this->CreateLink($id, 'defaultadmin', $returnid, '<', array('pagenumber' => $pagenumber - 1,
                'active_tab' => 'itemtab')));
    $smarty->assign('firstpage', $this->CreateLink($id, 'defaultadmin', $returnid, '<<', array('pagenumber' => 1,
                'active_tab' => 'itemtab')));
}
if ($pagenumber >= $pagecount) {
    ...
    $smarty->assign('nextpage', $this->CreateLink($id, 'defaultadmin', $returnid, '>', array('pagenumber' => $pagenumber + 1,
                'active_tab' => 'itemtab')));
    $smarty->assign('lastpage', $this->CreateLink($id, 'defaultadmin', $returnid, '>>', array('pagenumber' => $pagecount,
                'active_tab' => 'itemtab')));
}
#Find Mact = LISESEO
...
$smarty->assign('sortable', $this->GetPreference('display_sortable', 0));
