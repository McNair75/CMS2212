...
<table id="{($sortable) ? 'sortable_item' :''}" data-filter="#item_search" cellspacing="0" class="pagetable {$themeObject->themeName}">
    <thead>
        <tr class="top">
            <th data-toggle="true">{if $mod->prefix == "liseseo"|lower}SKU{else}{$title}{/if}</th>
    ...
    <tbody class="content" width="100%">
        {foreach from=$items item=entry}
            ...
            <tr id="item_{$entry->item_id}" class="{$rowclass}" style="{($sortable) ? 'cursor: move' :''}">
                <td>{$entry->title}</td>
