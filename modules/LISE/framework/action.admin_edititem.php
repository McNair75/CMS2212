...
if (!$adminuser && !$this->VerifyOwner(lise_utils::init_var('item_id', $params, -1))) {
...
if (isset($params['customfield'])) {
    if (!is_array($params['customfield'])) {
        $params['customfield'] = array($params['customfield']);
    }
...
if (isset($params['submit']) || isset($params['apply']) || isset($params['save_create'])) {
    ...
    if (empty($alias)) {
        $alias = munge_string_to_url($title, true, true);
    } else {
        $alias = munge_string_to_url($alias, true, true);
    }

    if (!$allow_item_sametitle) {
        // Check for duplicate
        if ($item_id > 0) {
    ...
    if (empty($errors)) {
        ...
        $obj->desc = $desc;
        $obj->alias = $alias;
        $obj->active = (!$adminuser) ? -1 : (isset($params['active']) ? 1 : 0);
