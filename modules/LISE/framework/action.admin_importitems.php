...
$import_values = lise_utils::init_var('import_values', $params, array());
...
foreach ($item as $k => $v) {
    ...
    if ($v instanceof LISEFielddefArray) {
        ...
        $obj->required_image = in_array($k, LISEItem::$mandatory) ? $admintheme->DisplayImage('icons/system/true.gif', '', '', '', 'systemicon') : $admintheme->DisplayImage('icons/system/false.gif', '', '', '', 'systemicon');
    ...
    if (empty($errors)) {
        ...
        foreach ($database_values as $item_values) {
            ...
            foreach ($item_values as $key => $value) {
                ...
                if ($key == 'item_id')
                    LISEItemOperations::Load($this, $obj); // <- Try loading item if we actually got ID.
            }
