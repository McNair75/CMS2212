...
class LISEItemQuery extends LISEQuery {
    ...
    private function _append_internal() {
        ...
        if (isset($params['orderby'])) {
            ...
            list($order_cols, $custom_cols) = self::explode_orderby($params['orderby'], self::$orderby_map);
            ...
            if (!empty($custom_cols)) {
                foreach ($custom_cols as $index => $obj) {
                    ...
                    if ($onedef) {
                        $as = 'VAL' . $onedef->GetId();
                        $this->AppendTo(parent::VARTYPE_JOINS, 'LEFT JOIN '
                                . cms_db_prefix()
                                . 'module_'
                                . $mod->_GetModuleAlias()
                                . '_fieldval '
                        ...
                        $this->AppendTo(parent::VARTYPE_WHERE, $as
                                . '.fielddef_id = '
        ...
        //+Lee
        if (!empty($params['fieldid']) && is_numeric($params['fieldid'])) {
            $str = " (F.fielddef_id = " . $params['fieldid'] . ")";
            $this->AppendTo(parent::VARTYPE_WHERE, '(IFIELD.`value` = "' . $params['fieldval'] . '")');
            $this->AppendTo(parent::VARTYPE_WHERE, $str);
        }

        if (!empty($params['fieldalias']) && !empty($params['fieldval'])) {
            $str = " (F.alias = '" . $params['fieldalias'] . "')";
            $this->AppendTo(parent::VARTYPE_WHERE, '(IFIELD.`value` = "' . $params['fieldval'] . '")');
            $this->AppendTo(parent::VARTYPE_WHERE, $str);
        }
    ...
    protected function _query() {
        ...
        // Init query
        $this->query = 'SELECT A.*,IFIELD.value as fieldtype FROM ' . cms_db_prefix() . 'module_' . $mod->_GetModuleAlias() . '_item A 
					...
					ON IB.category_id = B.category_id 
                                        LEFT JOIN ' . cms_db_prefix() . 'module_' . $mod->_GetModuleAlias() . '_fieldval IFIELD 
					ON A.item_id = IFIELD.item_id 
                                        LEFT JOIN ' . cms_db_prefix() . 'module_' . $mod->_GetModuleAlias() . '_fielddef F 
					ON IFIELD.fielddef_id = F.fielddef_id ';
        ...
        if (($this->totalcount % $pagelimit) != 0)
            $this->pagecount++;
