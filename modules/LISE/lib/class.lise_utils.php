...
class lise_utils 
{
  ...
  */
  private static function _to_alias_format($str)
  {
    ...
    {
      if(!is_string($prefix)) $prefix = 'item';
      if(self::StartsWithNumber($prefix) ) $prefix = 'item';
  ...
  */
  public static final function generate_unique_alias(LISE $mod, $name)
  {
    ...
    if(self::StartsWithNumber($alias) ) return FALSE;
  ...
  public static function log($message = '')
  {
    ...
    foreach($dbt as $one)
    {
      ...
      if( !is_a($one['object'], 'LISE' ) ) continue;
