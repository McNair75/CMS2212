...
class LISEColorListQuery extends LISEQuery {
    #---------------------
    # Attributes
    #---------------------
    ...
    private static $orderby_map = array(
        // color
        'color_id' => 'color_id', 'color_code' => 'code', 'name' => 'color_name', 'alias' => 'color_name', 'hierarchy' => 'hierarchy', 'position' => 'position',
    ...
    private function _append_internal() {
        ...
        if (isset($params['orderby'])) {
            ...
            list($order_cols, $custom_cols) = self::explode_orderby($params['orderby'], self::$orderby_map); // <- Bit broken, wasn't ment for color action, second array totally unneccery.
    ...
    protected function _query() {
        ...
        // Init query
        $this->query = 'SELECT * FROM ' . cms_db_prefix() . 'module_' . $mod->_GetModuleAlias() . '_color B ';
        ...
        $this->query .= ' GROUP BY B.color_id ';
        ...
        if ($params['orderby'] && $params['sortby']) {
            ...
            if (count($this->orderby)) {
                ...
                $this->query .= ' ORDER BY B.color_name ASC';
            }
        }
