...
class LISE extends CMSModule {
    ...
    private function _autoloader($classname) {
        ...
        $fn = cms_join_path(
                $this->GetModulePath(), 'lib', 'class.' . $classname . '.php'
        );
    ...
    public function GetChangeLog() {
        ...
        $fn = cms_join_path(
                dirname(__FILE__), 'changelog.html'
        );
...
INNER JOIN cms_modules AS m ON i.module_name = m.module_name';
    ...
    public function GetFieldDefs($include_list = array()) {
        ...
        if (isset($this->_fielddef_cache)) {
            ...
            foreach ($this->_fielddef_cache as $field) {
                if (
                        count($include_list) && !in_array($field->GetAlias(), $include_list) && !in_array($field->GetId(), $include_list)
                ) {
                    continue;
                }
    ...
    public function DeleteItemById($id, $mleblock) {
        $obj = $this->LoadItemByIdentifier('item_id', $id, $mleblock);
        LISEItemOperations::Delete($this, $obj);
    }

    public function VerifyOwner($item_id) {
        $db = cmsms()->GetDb();
        $loggedin = get_userid(false);
        $query = 'SELECT owner FROM ' . cms_db_prefix() . 'module_' . $this->_GetModuleAlias() . '_item WHERE item_id = ?';
        $result = $db->GetOne($query, array($item_id));
        if ($result == $loggedin) {
            return true;
        }
    ...
    public function _getOptionValueByFielddefIdName($sid, $name) {
        $db = cmsms()->GetDb();
        $query = 'SELECT `value` FROM ' . cms_db_prefix() . 'module_' . $this->_GetModuleAlias() . '_fielddef_opts WHERE (`fielddef_id` = ? AND `name` = ?)';
        $value = $db->GetOne($query, array($sid, $name));
    ...
    // ColorList methods
    //---------------------
    public function SaveColorList(LISEColorList &$obj, $mleblock) {
        LISEColorListOperations::Save($this, $obj, $mleblock);
    }

    public function InitiateColorList() {
        return new LISEColorList();
    }

    public function LoadColorListByIdentifier($identifier, $value, $mleblock = '') {
        if (!in_array($identifier, LISEColorListOperations::$identifiers)) {
            throw new \LISE\Exception("Illegal identifier: $identifier!");
        }
        $obj = $this->InitiateColorList();
        $obj->$identifier = $value;
        LISEColorListOperations::Load($this, $obj, '', $mleblock);

        return $obj;
    }

    public function DeleteColorListById($id) {
        $obj = $this->LoadColorListByIdentifier('color_id', $id);
        LISEColorListOperations::Delete($this, $obj);
    }

    // Make bulk action (using this: action.ajax.php)
    public function SaveColorListOrder($color_order) {
        $db = cmsms()->GetDb();
        $query = 'UPDATE ' . cms_db_prefix() . 'module_' . $this->_GetModuleAlias() . '_color SET position = ? WHERE color_id = ?';
        foreach ($color_order as $key => $item) {
            $db->Execute($query, array($key, $item));
        }
        LISEColorListOperations::UpdateHierarchyPositions($this);
        return $this->ModLang('changessaved');
    }

    public function GetColorListQuery(&$params) {
        $class = $this->_GetModuleConfig('colorlist_query_class');
        return new $class($this, $params);
    }
    ...
    public function CreatePrettyLink(
    $id, $action, $returnid = '', $contents = '', $params = array(), $warn_message = '', $onlyhref = false, $inline = false, $addtext = '', $targetcontentonly = false, $prettyurl = ''
    ) {
        switch ($action) {
            case 'detail':
            ...
            case 'default':
                ...
                //+Lee fieldalias,fieldval
                if (isset($params['fieldalias']) && isset($params['fieldval'])) {
                    $string_array[] = $params['fieldalias'];
                    $string_array[] = $params['fieldval'];
                }
