...
if( isset($params['resettodefault']) &&
    ...
    $params['prefname'] = filter_var($params['prefname'],FILTER_SANITIZE_STRING);
    $params['filename'] = filter_var($params['filename'],FILTER_SANITIZE_STRING);
    if( $params['filename'] ) $params['filename'] = basename($params['filename']);
    if( $params['prefname'] && $params['filename'] ) {
        $fn = $module->GetModulePath().'/templates/'.$params['filename'];
        if( is_file( $fn ) ) {
...
$the_tab = cge_param::get_string($params,'cg_activetab');
...
$module->RedirectToAdminTab($the_tab);
