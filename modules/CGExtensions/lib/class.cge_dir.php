...
final class cge_dir
{
     ...
     */
    public static function recursive_rmdir(string $directory)
    {
     ...
     * @param string $directory The absolute path to the directory to be removed.
     */
    public static function recursive_remove_directory(string $directory)
    {
     ...
     */
    static public function dir_list(string $parent)
    {
     ...
     */
    public static function file_list_regexp(string $dir,string $regexp,int $limit=1000000)
    {
     ...
     * @return bool
     */
    public static function mkdirr (string $pathname,int $mode = 0755,callable $callback = null) : bool
    {
        // Check if directory already exists
        if (empty($pathname) || is_dir ($pathname)) return TRUE;

        // Ensure a file does not already exist with the same name
        if (is_file ($pathname)) return TRUE; // RC: Modification such that this isn't an error

        // Crawl up the directory tree
        $next_pathname = dirname ($pathname);
        if (self::mkdirr ($next_pathname, $mode)) {
            if (!is_file ($pathname)) {
                $res = mkdir ($pathname, $mode);
                if( $res && is_callable($callback) ) call_user_func($callback,$pathname);
                return $res;
            }
        }
     ...
     * @return string[]
     */
    public static function get_file_list(string $dir,string $extensions,bool $sorted = true)
    {
        $filetypes = array();
        ...
        if( $dh ) {
            while (false !== ($file = readdir($dh))) {
                ...
                if( !empty($filetypes) > 0 && !in_array( $ext, $filetypes ) ) continue;
     ...
     */
    public static function file_matches_pattern(string $filename,string $pattern,bool $case_sensitive = TRUE,bool $allow_empty = TRUE)
    {
        if( !is_array($pattern) ) {
     ...
     */
    public static function recursive_glob(string $path,$pattern,string $mode = 'FULL',$excludepattern = '',int $maxdepth = -1,int $d = 0)
    {
