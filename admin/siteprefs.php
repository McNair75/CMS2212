...
$fn = dirname(__FILE__) . '/themes_under/default/default.tpl';
$themes_under = @file_get_contents($fn);
...
$countdown_timer = '1987/06/27';
$lock_timeout = 60;
$use_wysiwyg = 1;
$sitedownmessage = $themes_under;
...
$countdown_timer = cms_siteprefs::get('countdown_timer', $countdown_timer);
...
if ($enablesitedownmessage) {
    // copy themes_under from admin to root
    copyr(dirname(__FILE__) . '/themes_under/default', cms_join_path($config['root_path'], $config['themes_url'], 'under'), array('default.tpl'));
}
    ...
    if ($access) {
        switch ($tab) {
            ...
            case 'sitedown':
                ...
                cms_siteprefs::set('countdown_timer', $countdown_timer);
...
if (
    FALSE == is_writable(TMP_CACHE_LOCATION) ||
...
$smarty->assign('countdown_timer', $countdown_timer);
...
$sitedownexcludes_array = [];
if (!empty($sitedownexcludes)) {
    $arr = explode(',', $sitedownexcludes);
    foreach ($arr as $item) {
        $sitedownexcludes_array[] = $item;
    }
}

$smarty->assign('sitedownexcludes_array', $sitedownexcludes_array);
