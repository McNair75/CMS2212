<div class="pagecontainer">
    ...
    <form id="siteprefform_sitedown" method="post" action="{$formurl}">
        ...
        {if $enablesitedownmessage}
            <div class="pageoverflow">
                <p class="pagetext"><label for="usewysiwyg">Timer CountDown:</label> </p>
                <p class="pageinput">
                    <input id="countdown_timer" type="text" name="countdown_timer" size="20" maxlength="255" value="{$countdown_timer|default:'2018/12/21'}" />
                </p>
            </div>
        {/if}
        ...
        <div class="pageoverflow">
            ...
            <p class="pageinput">
                ...
                {if $sitedownexcludes_array}
                    <br/> 
                <ul>
                    {foreach from=$sitedownexcludes_array item=$ip}
                        <li>{$ip}</li>
                        {/foreach}
                </ul>
            {/if}
    ...
    <div id="testpopup" title="{lang('title_mailtest')}" style="display: none;">
        <form id="siteprefform_mail_1" method="post" action="{$formurl}">
            <div>
                <input type="hidden" name="{$SECURE_PARAM_NAME}" value="{$CMS_USER_KEY}"/>
    ...
    <form id="siteprefform_mail_2" method="post" action="{$formurl}">
        <div>
            <input type="hidden" name="{$SECURE_PARAM_NAME}" value="{$CMS_USER_KEY}"/>
            <input type="hidden" name="active_tab" value="mail" />
            <input type="hidden" name="editsiteprefs" value="true" />
    ...
    <form id="siteprefform_setup_smarty" method="post" action="{$formurl}">
        <div>
            <input type="hidden" name="{$SECURE_PARAM_NAME}" value="{$CMS_USER_KEY}"/>
